...
CvMat*  GetMat2( FILE *fh, int isVector )
{
...
CvMat*  GetMat( FILE *fh, int isVector )
{
...
int WriteMat(CvMat* m, FILE *fh, int isVector )
{
    FILE *Stream;					//Stream for output.
...
int WriteMat2(CvMat* m, FILE *fh, int isVector )
{
    FILE *Stream;					//Stream for output.
    int cIndex=2;					//Counter for elements in the matrix.
...
int main(int argc, char *argv[])
{
    ...
    if (argc<2)
        ...
        switch(argv[1][0])
        {
            case 'e':
                {
                    IplImage* img= cvLoadImage(argv[2],-1);
                    if(img==0)
                    {
